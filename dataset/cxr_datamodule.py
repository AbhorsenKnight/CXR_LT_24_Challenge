    ...
    def setup(self, stage):
        ...
        if stage in ('fit', 'validate'):
            ...
            if self.cfg["predict_pseudo_label"] == "vinbig":
                ...
                print(self.cfg["pred_df_path"])
                pred_df = pd.read_csv(self.cfg["pred_df_path"])
                print(len(pred_df))
                #self.pred_dataset = CxrStudyIdDataset(self.cfg, pred_df, transforms_val)
                if self.cfg['stage'] == 'single':
                    self.pred_dataset = CxrDataset(self.cfg, pred_df, transforms_val)
                elif self.cfg['stage'] == 'fusion' or self.cfg['stage'] == 'fusion_task2' or self.cfg['stage'] == 'fusion_task3':
                    self.pred_dataset = CxrStudyIdDataset(self.cfg, pred_df, transforms_val)
